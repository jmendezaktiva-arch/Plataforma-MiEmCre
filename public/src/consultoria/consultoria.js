...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const renderServices = () => {
        ...
        productsContainer.innerHTML = staffServices.map(service => {
            ...
            return `
            <section class="card bento-item glass-card bento-kpi-card card-lobby-item animate-fade-in" 
                     style="position: relative; border-top: 4px solid var(--accent-gold);">
                <div class="card-content" style="display: flex; flex-direction: column; height: 100%; width: 100%;">
                    ...
                    <div class="card-body-app" style="flex-grow: 1; margin-bottom: 25px;">
                        <div class="roadmap-link" data-roadmap-target="popover-${service.id}">
                            <span>+ VER ROADMAP ESTRATÉGICO</span>
                        </div>
                        
                        ${service.purposeDesc ? `
                            <div class="description-popover" id="popover-${service.id}">
                                <p><strong>Nuestra Promesa:</strong><br>${service.purposeDesc}</p>
                            </div>
                        ` : ''}
                    </div>

                    <footer class="card-footer-app" style="margin-top: auto;">
                        <button class="btn-primary" data-action="request" data-id="${service.id}" style="font-size: var(--font-size-button); padding: 16px; font-weight: 800; letter-spacing: 0.5px;">
                            SOLICITAR ACCESO Y ZOOM
                        </button>
                    </footer>
                </div>
            </section>`;
        ...
        // --- MOTOR DE FOCO PRESTIGE (QUIRÚRGICO) ---
        setTimeout(() => {
            const cards = productsContainer.querySelectorAll('.card-lobby-item');
            cards.forEach((card) => {
                card.addEventListener('mouseenter', () => {
                    cards.forEach(c => {
                        ...
                        if (c !== card) {
                            ...
                            body?.classList.remove('card-content-visible');
                            footer?.classList.remove('card-content-visible');
                        } else {
                            c.classList.add('card-active', 'card-expanded');
                            c.classList.remove('card-inactive');
                            body?.classList.add('card-content-visible');
                            footer?.classList.add('card-content-visible');
                        }
                    });
                });
    ...
    document.addEventListener('click', (e) => {
        // --- MOTOR DE POP-OVERS DE ROADMAP (CONSULTORÍA) ---
        ...
        if (!e.target.closest('.description-popover') && !e.target.closest('.roadmap-link')) {
            document.querySelectorAll('.description-popover.active').forEach(p => p.classList.remove('active'));
        }
        ...
        if (action === 'request') {
            ...
            const notificarIntervencion = async () => {
                ...
                try {
                    ...
                    // TRACEABILIDAD: Usamos await para asegurar que el servidor reciba la señal antes de avisar al usuario
                    await fetch('/.netlify/functions/intervencion-notificacion', {
                        ...
                        body: JSON.stringify({
                            ...
                            cliente: { 
                                ...
                                empresa: leadData.empresa // Inyección crítica para el Prestige Email
                            },
