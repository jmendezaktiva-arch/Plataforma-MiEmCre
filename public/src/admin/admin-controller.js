...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    /**
     * MOTOR DE UI DINÁMICA: Filas de KPIs (Estrategia Multidimensional)
     * Centralizado fuera de listeners para disponibilidad inmediata.
     */
    const KPIEngine = {
        renderRow(data = { label: '', field: '', unit: '' }) {
            const rowId = `kpi-row-${Date.now()}-${Math.random().toString(16).slice(2)}`;
            return `
            <div class="kpi-config-row" id="${rowId}" style="display: grid; grid-template-columns: 1fr 1fr 0.5fr 30px; gap: 8px; align-items: end; background: rgba(15, 52, 96, 0.03); padding: 10px; border-radius: 12px; border: 1px solid rgba(15, 52, 96, 0.08); margin-bottom: 8px;">
                <div>
                    <label style="font-size: 0.55rem; color: #999; display: block; font-weight: 700;">ETIQUETA:</label>
                    <input type="text" class="form-control kpi-label" value="${data.label}" placeholder="Ej: Ventas" style="margin: 0; font-size: 0.7rem; padding: 8px;">
                </div>
                <div>
                    <label style="font-size: 0.55rem; color: #999; display: block; font-weight: 700;">ID CAMPO (FIREBASE):</label>
                    <input type="text" class="form-control kpi-field" value="${data.field}" placeholder="kpi_ventas" style="margin: 0; font-size: 0.7rem; padding: 8px;">
                </div>
                <div>
                    <label style="font-size: 0.55rem; color: #999; display: block; font-weight: 700;">UNIDAD:</label>
                    <input type="text" class="form-control kpi-unit" value="${data.unit}" placeholder="%, $" style="margin: 0; font-size: 0.7rem; padding: 8px;">
                </div>
                <button type="button" onclick="document.getElementById('${rowId}').remove()" style="background:none; border:none; color:#cc0000; cursor:pointer; font-size:1.2rem; padding-bottom:5px; line-height: 1;">&times;</button>
            </div>`;
        },
        
        getKpisFromContainer(containerId) {
            const container = document.getElementById(containerId);
            if (!container) return [];
            return Array.from(container.querySelectorAll('.kpi-config-row')).map(row => ({
                label: row.querySelector('.kpi-label').value,
                field_source: row.querySelector('.kpi-field').value,
                unit: row.querySelector('.kpi-unit').value
            })).filter(k => k.field_source);
        }
    };

    // Listener Maestro: Se activa inmediatamente con la carga del DOM
    document.addEventListener('click', (e) => {
        const btn = e.target.closest('#btn-add-kpi-row');
        if (btn) {
            e.preventDefault();
            const container = document.getElementById('kpis-container');
            if (container) container.insertAdjacentHTML('beforeend', KPIEngine.renderRow());
        }
    });
    ...
    formCurso?.addEventListener('submit', async (e) => {
        ...
        const courseData = {
            ...
            purposeTitle: document.getElementById('course-purpose-title').value,
            purposeDesc: document.getElementById('course-purpose-desc').value,
            ...
            // [MANTENEMOS LOGICA MANUAL]: Tú escribes los IDs, el motor los captura todos
            kpis: KPIEngine.getKpisFromContainer('kpis-container'),
            
            accentColor: document.getElementById('course-accent-color')?.value || '#0F3460',
            buttonText: document.getElementById('course-button-text')?.value || 'Iniciar Programa',
    ...
    window.prepareEditCourse = async (id) => {
        try {
            ...
            if (docSnap.exists()) {
                ...
                // [NUEVO] Mapeo Inverso Multi-KPI (Reconstrucción Dinámica de Estrategia)
                const kpisContainer = document.getElementById('kpis-container');
                if (kpisContainer) {
                    kpisContainer.innerHTML = ''; // Limpieza quirúrgica para evitar duplicados en el DOM
                    
                    if (c.kpis && Array.isArray(c.kpis) && c.kpis.length > 0) {
                        // Renderizamos cada métrica guardada en el arreglo multidimensional
                        c.kpis.forEach(k => {
                            kpisContainer.insertAdjacentHTML('beforeend', KPIEngine.renderRow({
                                label: k.label,
                                field: k.field_source,
                                unit: k.unit
                            }));
                        });
                    } else if (c.kpi_config) { 
                        // Mecanismo de Resiliencia: Migración automática de datos del modelo anterior
                        kpisContainer.insertAdjacentHTML('beforeend', KPIEngine.renderRow({
                            label: c.kpi_config.label,
                            field: c.kpi_config.field_source,
                            unit: c.kpi_config.unit
                        }));
                    }
                }

                // Sincronización de Identidad Visual (Mapeo Inverso)
                const accentColor = c.accentColor || '#0F3460';
                const elColorPicker = document.getElementById('course-accent-color');
                const elColorHex = document.getElementById('course-accent-hex');
                const elBtnText = document.getElementById('course-button-text');

                if (elColorPicker) elColorPicker.value = accentColor;
                if (elColorHex) elColorHex.value = accentColor;
                if (elBtnText) elBtnText.value = c.buttonText || 'Iniciar Programa';
    ...
    formServicio?.addEventListener('submit', async (e) => {
        ...
        const serviceData = {
            ...
            // [NUEVO] Configuración de KPI de Consultoría
            kpi_config: {
                label: document.getElementById('service-kpi-label')?.value || '',
                field_source: document.getElementById('service-kpi-field')?.value || '',
                unit: document.getElementById('service-kpi-unit')?.value || ''
            },
    ...
    window.prepareEditService = async (id) => {
        try {
            ...
            if (docSnap.exists()) {
                ...
                // [NUEVO] Mapeo Inverso de KPIs de Consultoría
                if (s.kpi_config) {
                    document.getElementById('service-kpi-label').value = s.kpi_config.label || '';
                    document.getElementById('service-kpi-field').value = s.kpi_config.field_source || '';
                    document.getElementById('service-kpi-unit').value = s.kpi_config.unit || '';
                }
            ...
            tableBody.innerHTML = querySnapshot.docs.map(docSnap => {
                ...
                <tr style="border-bottom: 1px solid #f4f4f4; transition: background 0.2s;" onmouseover="this.style.background='#fafafa'" onmouseout="this.style.background='transparent'">
                    ...
                    <td style="padding: 15px;">
                        ...
                        ` : req.estado === 'agendado' ? `
                            <span style="background: #e0f2fe; color: #0369a1; padding: 4px 10px; border-radius: 6px; font-size: 0.65rem; font-weight: 800; text-transform: uppercase; border: 1px solid #7dd3fc;">
                                📅 AGENDADO
                            </span>
                        ` : `
