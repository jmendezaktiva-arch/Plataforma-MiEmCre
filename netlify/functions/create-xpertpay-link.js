...
exports.handler = async (event) => {
    ...
    try {
        ...
        // TRACEABILIDAD: Generación de ID numérico único con espacio de 1 billón (9 dígitos)
        const normalizedId = String(courseId).trim().toLowerCase();
        const hash = normalizedId.split('').reduce((a, b) => {
            a = ((a << 5) - a) + b.charCodeAt(0);
            return a & a; // Fuerza a entero de 32 bits
        }, 0);

        // Usamos un módulo de 1,000,000,000 para asegurar un ID de hasta 9 dígitos (Seguro para XpertPay)
        const idNumericoSimple = Math.abs(hash) % 1000000000;

        console.log(`🆔 Dreams Sales: Mapping slug [${courseId}] to numeric ID [${idNumericoSimple}]`);
